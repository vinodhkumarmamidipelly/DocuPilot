// Simple utility to create example Word documents
// Compile: csc CreateWordFiles.cs /r:"C:\path\to\DocumentFormat.OpenXml.dll"
// Run: CreateWordFiles.exe
...
class Program
{
    static void Main()
    {
        ...
        var rawPath = Path.Combine(baseDir, "example_proper_raw.docx");
        var templatePath = Path.Combine(baseDir, "example_proper_template.dotx");

        Console.WriteLine("Creating example_proper_raw.docx...");
        CreateRawDocx(rawPath);
        Console.WriteLine($"✓ Created: {rawPath}");

        Console.WriteLine("Creating example_proper_template.dotx...");
        CreateTemplateDotx(templatePath);
        Console.WriteLine($"✓ Created: {templatePath}");

        Console.WriteLine("\nDone!");
    }

    static void CreateRawDocx(string path)
    {
    ...
    static void CreateTemplateDotx(string path)
    {
    ...
    static void AddNumberedItem(Body body, string text)
    {
        var para = new Paragraph();
        para.ParagraphProperties = new ParagraphProperties(
            new NumberingProperties(
