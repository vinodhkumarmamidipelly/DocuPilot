...
namespace SMEPilot.FunctionApp.TestFiles
{
    ...
    {
        public static void CreateExampleRawDocx(string outputPath)
        {
        ...
        public static void CreateExampleTemplateDotx(string outputPath)
        {
        ...
        private static Styles CreateStyles()
        {
            ...
            for (int i = 1; i <= 4; i++)
            {
                ...
                var runProps = new StyleRunProperties(
                    new RunFonts() { Ascii = "Calibri", HighAnsi = "Calibri" },
                    new Bold(),
                    new FontSize() { Val = ((44 - (i * 2)) * 2).ToString() } // Decreasing size
                );
